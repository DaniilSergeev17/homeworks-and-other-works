...
threshold_for_token_in_moe = 0.2 # threshold for choosing token by each expert in MoE (Expert Choice)
# -------------
...
class MixtureOfExperts(nn.Module):
  ...
  def forward(self, x):
      ...
      # get tokens probabilities for each expert
      token_probs = nn.functional.softmax(gate_output, dim=1)  # Batch*(block_size), Channels (num_experts)

      final_output = torch.zeros_like(inputs_flattened)  # Batch*(block_size), Channels (n_embd)

      # Forward pass through the experts, which choose tokens
      for i, expert in enumerate(self.experts):
          # get token probs for current expert
          expert_token_probs = token_probs[:, i]  # probs for expert i

          # select tokens with threshold for current expert
          selected_token_mask = expert_token_probs > threshold_for_token_in_moe
          selected_token_indices = torch.where(selected_token_mask)[0]

          if len(selected_token_indices) > 0:
              expert_output = expert(inputs_flattened[selected_token_indices])
              final_output[selected_token_indices] += expert_token_probs[selected_token_indices, None] * expert_output
              # we added additional dim (as None in expert_token_probs to multiply properly)

      return final_output.view_as(x)  # (batch_size, block_size, n_embd)
...
class SimpleLLM(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.blocks(x) # (Batch, block_size, n_embd)
